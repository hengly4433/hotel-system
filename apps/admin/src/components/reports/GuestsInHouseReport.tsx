...
export default function GuestsInHouseReport({ date }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await reportsApi.getGuestsInHouse(date ? format(date, "yyyy-MM-dd") : undefined);
        if (active) {
          setData(result);
          setPage(0);
        }
      } catch (err) {
        console.error("Failed to fetch guests in house", err);
      } finally {
