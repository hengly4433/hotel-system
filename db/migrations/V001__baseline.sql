...
CREATE TABLE IF NOT EXISTS room_types (
  ...
  max_occupancy  int  NOT NULL DEFAULT 1 CHECK (max_occupancy >= 0),
  base_description text NULL,
  default_bed_type text NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_room_types_property_code_active
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_rooms_property_number_active
...
CREATE TABLE IF NOT EXISTS room_operational_status (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  room_id       uuid NOT NULL REFERENCES rooms(id) ON DELETE CASCADE,
  status        room_op_status NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_room_status_room ON room_operational_status(room_id, effective_from DESC);

CREATE TRIGGER trg_room_operational_status_updated_at
BEFORE UPDATE ON room_operational_status
FOR EACH ROW EXECUTE FUNCTION set_updated_at();

-- =========================
-- RBAC (Users → Roles → Submenus → Permissions)
-- =========================
