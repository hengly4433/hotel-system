...
import com.yourorg.hotel.reservation.domain.ReservationTypeNightEntity;
...
public interface ReservationTypeNightRepository extends JpaRepository<ReservationTypeNightEntity, UUID> {
  ...
  @Query(
      value = "select rtn.room_type_id, rtn.date, count(rtn.id) " +
          ...
          "group by rtn.room_type_id, rtn.date",
      nativeQuery = true
  )
